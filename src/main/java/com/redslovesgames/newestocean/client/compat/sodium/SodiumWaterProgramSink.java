...
/** Direct OpenGL uniform sink for Sodium's already-bound chunk shader program. */
final class SodiumWaterProgramSink implements OceanWaterUniformBinder.Sink {
    private static final int SHORE_TEXTURE_UNIT = 11;

    private final int programId;
    private boolean missingUniform;

    SodiumWaterProgramSink(int programId) {
        if (programId <= 0) {
            throw new IllegalArgumentException("Sodium shader program id must be positive");
        }
